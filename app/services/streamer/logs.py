"""Stream isi file log ke client lewat WebSocket.

Urusannya sama sekali lain dari stream prediksi: tidak ada model, tidak ada
poll ke SL, tidak ada dataset yang wajib ada. Yang dikerjakan hanya mengirim
riwayat log terakhir lalu mengikuti file-nya dengan watchdog dan meneruskan
tiap baris baru.
"""
import asyncio
import pathlib
...
from app.logger import LOGGER_GLOBAL, LogManager


class LogFileHandler(FileSystemEventHandler):
  """Handler untuk memantau perubahan file log menggunakan watchdog"""

  def __init__(self, callback: Callable[[str], None], filename: str = "main.log"):
  ...
  def on_modified(self, event):
    if event.is_directory:
      return
    # satu folder dataset berisi main.log DAN worker.log — hanya file yang
    # sedang di-tail yang boleh memicu pengiriman.
...
class LoggerState:
  """State bersama antar koneksi log.

  Di-key per FILE log (bukan per dataset): satu dataset punya main.log dan
  worker.log yang masing-masing butuh observer + queue sendiri.
  """
  ...
  async def add_connection(self, key: str, connection_id: str) -> bool:
    """Koneksi baru. True kalau ini yang pertama → watchdog harus dinyalakan."""
    async with self._lock:
      if key not in self._active_loggers:
        self._active_loggers[key] = set()
        self._message_queues[key] = asyncio.Queue()
        self._active_loggers[key].add(connection_id)
        return True
      self._active_loggers[key].add(connection_id)
      return False

  async def remove_connection(self, key: str, connection_id: str) -> bool:
    """Lepas koneksi. True kalau sudah tidak ada penonton → watchdog dimatikan."""
    async with self._lock:
      if key in self._active_loggers:
        self._active_loggers[key].discard(connection_id)
        if not self._active_loggers[key]:
          del self._active_loggers[key]
          if key in self._message_queues:
            del self._message_queues[key]
          return True
      return False

  def get_message_queue(self, key: str) -> Optional[asyncio.Queue]:
    """Queue baris log untuk satu file."""
    return self._message_queues.get(key)

  def add_observer(self, key: str, observer: Observer, handler: LogFileHandler):
    """Simpan observer + handler untuk satu file."""
    self._observers[key] = observer
    self._handlers[key] = handler

  def remove_observer(self, key: str):
    """Hentikan dan lepas observer untuk satu file."""
  ...
  def get_connection_count(self, key: str) -> int:
    """Jumlah koneksi yang sedang menonton file ini."""
    return len(self._active_loggers.get(key, set()))
...
class LogStreamer:
  ...
  def _get_log_file_path(self) -> Optional[pathlib.Path]:
    """File log yang di-tail. LogManager yang menentukan lokasinya supaya
    pembaca dan penulis tidak pernah menunjuk path berbeda; "system" jatuh ke
    global.log."""
    return LogManager.log_path(self.dataset_name, self.channel)

  @property
  def _state_key(self) -> str:
    """Kunci state per FILE, bukan per dataset: satu dataset punya main.log dan
    worker.log, dan keduanya butuh observer + queue sendiri. Kalau dikunci per
    dataset, penonton worker.log akan menerima baris main.log."""
  ...
  async def setup(self) -> bool:
    ...
    try:
      if self.require_dataset:
        # get_db_session(), bukan get_session(): yang terakhir itu generator —
        # mengopernya sebagai `db` membuat query gagal begitu cabang ini dipakai.
        with get_db_session() as db:
          dataset = Dataset.get_by_name(name=self.dataset_name, db=db)
        if not dataset:
  ...
  async def _send_initial_logs(self):
    ...
    try:
      logs = read_logs(self.dataset_name, self.channel)
  ...
  def _on_log_file_change(self, parsed_line: dict):
    ...
    queue = logger_state.get_message_queue(self._state_key)
    if queue:
      try:
  ...
  async def _start_file_watcher(self) -> bool:
    ...
    try:
      ...
      logger_state.add_observer(self._state_key, observer, handler)
  ...
  async def run(self) -> None:
    ...
    try:
      is_primary = await logger_state.add_connection(self._state_key, self.connection_id)
      ...
      message_queue = logger_state.get_message_queue(self._state_key)
      ...
      self._is_running = False
      should_stop_watcher = await logger_state.remove_connection(self._state_key, self.connection_id)

      if should_stop_watcher:
        logger_state.remove_observer(self._state_key)
